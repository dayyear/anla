    ...
    for (const auto& email : email_info[0]) {
        if (email[8].asInt() == Mod_Email_Base::UNGET) {
            ...
            if (data[0].asInt() != Mod_Email_Base::SUCCESS) {
                common::log(boost::str(boost::format("【邮箱】收取附件失败，result[%1%]") % data[0]));
                return;
            }
            std::ostringstream oss;
            for (const auto& item : email[7])
                oss << "[" << db.get_item_type(version.c_str(), item[0].asInt())["name"] << "×" << item[1] << "]，";
            common::log(boost::str(boost::format("【邮箱】收取附件 %1%") % oss.str().substr(0, oss.str().size() - 2)));
        }
        data = this->Mod_Email_Base_delete_email(Mod_Email_Base::ONE, email[0].asInt());
        if (data[0].asInt() != Mod_Email_Base::SUCCESS) {
            common::log(boost::str(boost::format("【邮箱】删除邮件 [%1%] 失败，result[%2%]") % common::utf2gbk(email[1].asString()) % data[0]));
            return;
        }
        common::log(boost::str(boost::format("【邮箱】删除邮件 [%1%]") % common::utf2gbk(email[1].asString())));
    }
