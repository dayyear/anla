...
void sxd_client::faction_god() {
    for (;;) {
        ...
        if (data[0].asInt() != Mod_Faction_Base::SUCCESS) {
            common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿¸ø°ïÅÉÉñÏñÉÏ [°×Ì´Ïã] Ê§°Ü£¬result[%1%]") % data[0]));
            return;
        }
        common::log("¡¾°ïÅÉ¡¿¸ø°ïÅÉÉñÏñÉÏ [°×Ì´Ïã]");
    }
}
...
void sxd_client::faction_roll_cake() {
    for (;;) {
        ...
        if (data[0].asInt() != Mod_Faction_Base::SUCCESS) {
            common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿¼ªÐÇ¸ßÕÕÊ§°Ü£¬msg[%1%]") % data[0]));
            return;
        }
        common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿¼ªÐÇ¸ßÕÕ£¬»ñµÃ [»ý·Ö¡Á%1%]£¬½ñÈÕÀÛ¼Æ [»ý·Ö¡Á%2%]") % data[2] % data[3]));
    }
}
...
void sxd_client::faction_lucky_wheel() {
    ...
    while (rosefinch_number) {
        ...
        if (data[0].asInt() != Mod_LuckyWheel_Base::SUCCESS) {
            common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿×ªÅÌ³é½±Ê§°Ü£¬result[%1%]") % data[0]));
            return;
        }
        //int type = data[1].asInt();
        rosefinch_number = data[2].asInt();
        //int item_id = data[3].asInt();
        common::log("¡¾°ïÅÉ¡¿×ªÅÌ³é½±");
        //common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿×ªÅÌ³é½±£¬»ñµÃ [%1%]") % db.get_code(version, "item", item_id)["text"]));
    }
}
...
void sxd_client::faction_join_feast() {
    ...
    if (data[0].asInt() == Mod_Faction_Base::SUCCESS) {
        ...
        common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿³ÔÏÉÑç»ñµÃ£º%1%") % boost::algorithm::join(items, "£¬")));
    }
}
...
void sxd_client::faction_approve() {
    ...
    for (const auto& request : request_list) {
        ...
        if (data[0].asInt() == Mod_Faction_Base::ACTION_SUCCESS)
            common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿ÉóºËÍ¨¹ý [%1%(%2%¼¶)] ¼ÓÈë") % common::utf2gbk(request[2].asString()) % request[4]));
    }
...
void sxd_client::faction_war() {
    ...
    if (can_ignup && !is_signup) {
        ...
        if (data[0].asInt() != Mod_FactionWar_Base::ACTION_SUCCESS) {
            common::log(boost::str(boost::format("¡¾°ïÅÉ¡¿°ïÅÉÕ½±¨ÃûÊ§°Ü£¬result[%1%]") % data[0]));
            return;
        }
        common::log("¡¾°ïÅÉ¡¿°ïÅÉÕ½±¨Ãû");
    }
}
