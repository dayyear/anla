...
void sxd_client::hero_mission() {
    ...
    for (int town_id : town_ids) {
        ...
        if (data[0].asInt() != Mod_HeroMission_Base::SUCCESS) {
            common::log(boost::str(boost::format("¡¾Ó¢ĞÛÉ¨µ´¡¿É¨µ´Ê§°Ü£¬msg[%1%]") % data[0]));
            return;
        }
        for (;;) {
        ...
        common::log(boost::str(boost::format("¡¾Ó¢ĞÛÉ¨µ´¡¿É¨µ´Ó¢ĞÛ¸±±¾[%1%]") % db.get_code(version, "Town", town_id)["text"]));
