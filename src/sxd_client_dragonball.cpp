...
void sxd_client::dragonball() {
    ...
    for (;;) {
        ...
        if (data[0].asInt() != Mod_Dragonball_Base::SUCCESS) {
            common::log(boost::str(boost::format("¡¾ÉñÁúÉÏ¹©¡¿ÆÕÍ¨ÉÏ¹©Ê§°Ü£¬result[%1%]") % data[0]));
            return;
        }
        common::log(boost::str(boost::format("¡¾ÉñÁúÉÏ¹©¡¿ÆÕÍ¨ÉÏ¹©£¬»ñµÃ [%1%]") % dragonball_names[data[1][0][1].asInt()]));
        ...
        for (const auto& tmp_grid_d_b : tmp_grid_d_b_list) {
            ...
            if (tmp_grid_d_b[1].asInt() >= 27 && tmp_grid_d_b[1].asInt() <= 34) {
                ...
                if (data[0].asInt() != Mod_Dragonball_Base::SUCCESS) {
                    common::log(boost::str(boost::format("¡¾ÉñÁúÉÏ¹©¡¿Âô³ö [%1%] Ê§°Ü£¬result[%2%]") % dragonball_names[tmp_grid_d_b[1].asInt()] % data[0]));
                    return;
                }
                common::log(boost::str(boost::format("¡¾ÉñÁúÉÏ¹©¡¿Âô³ö [%1%]") % dragonball_names[tmp_grid_d_b[1].asInt()]));
            } else {
                data = this->Mod_Dragonball_Base_collect_dragonball(&id, 1);
                if (data[0].asInt() != Mod_Dragonball_Base::SUCCESS) {
                    common::log(boost::str(boost::format("¡¾ÉñÁúÉÏ¹©¡¿Ê°È¡ [%1%] Ê§°Ü£¬result[%2%]") % dragonball_names[tmp_grid_d_b[1].asInt()] % data[0]));
                    return;
                }
                common::log(boost::str(boost::format("¡¾ÉñÁúÉÏ¹©¡¿Ê°È¡ [%1%]") % dragonball_names[tmp_grid_d_b[1].asInt()]));
            }
        }
    }
}
