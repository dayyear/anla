...
sxd_web_ssl::sxd_web_ssl() :
        ssl_context(boost::asio::ssl::context::sslv23_client), ssl_stream(ios, ssl_context) {
    ssl_stream.set_verify_mode(boost::asio::ssl::verify_peer);
    ssl_stream.set_verify_callback([this]( bool preverified, boost::asio::ssl::verify_context& context)->bool {return on_peer_verify(preverified, context);});
}

sxd_web_ssl::~sxd_web_ssl() {
}

void sxd_web_ssl::connect(const std::string& host, const std::string& port) {
    this->host = host;
    this->port = port;
    boost::asio::ip::tcp::resolver resolver(ios);
    boost::asio::ip::tcp::resolver::query query(host, port);
    boost::asio::connect(ssl_stream.lowest_layer(), resolver.resolve(query));
    ssl_stream.handshake(boost::asio::ssl::stream_base::client);
}

void sxd_web_ssl::service(const std::string& method, const std::string& request_uri, const std::string& request_http_version, const std::string& request_header, const std::string& content, std::string& response_http_version, unsigned int& status_code, std::string& status_message, std::string& response_header, std::string& response_body) {
    // -------------------------------------------------------------------------------------------
    // 0. prepare
    // -------------------------------------------------------------------------------------------
    // method
    ...
    if (transfer_encoding == "chunked") {
        ...
        do {
            ...
            if (chunk_size + 2 > response.size())
                boost::asio::read(ssl_stream, response, boost::asio::transfer_exactly(chunk_size + 2 - response.size()));
        ...
        if ((size_t) content_length > response.size())
            boost::asio::read(ssl_stream, response, boost::asio::transfer_exactly(content_length - response.size()));
...
std::string sxd_web_ssl::get(const std::string& request_uri, const std::string& request_header) {
...
std::string sxd_web_ssl::post(const std::string& request_uri, const std::string& request_header, const std::string& content){
    std::string response_http_version;
    unsigned int status_code;
    std::string status_message;
    std::string response_header;
